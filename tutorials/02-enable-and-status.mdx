...
Since v0.3.3, including current v0.4.1, the SDK uses background polling by default. You typically don't need to call `poll_feedback_once()`.

```python
# Recommended pattern in current releases
motor.request_feedback()
state = motor.get_state()
```
