...
|--------|------------|-------------------|-----------------|
| Damiao | `add_damiao_motor()` | SocketCAN, DM Serial, DM-Device | DM-J4310/4340 |
| RobStride | `add_robstride_motor()` | SocketCAN | RobStride Extended CAN |
| RobStride (CiA402, exp.) | `add_robstride_motor(..., vendor="robstride_cia402")` | SocketCAN | CANopen/CiA402 (`F_CMD=1`) |
| RobStride (MIT, exp.) | `add_robstride_motor(..., vendor="robstride_mit")` | SocketCAN | RobStride MIT (`F_CMD=2`) |
...
| RobStride | rs-00 through rs-06 | ✅ | ✅ (= PP; CSP variant) | ✅ | ❌ |
| MyActuator | X8 | ❌ | ✅ | ✅ | ❌ |
...
- **PP / CSP:** Unified `pos-vel` is already PP (via `limit_spd`). For full PP parameters (`vel_max` + `acc_set`) or CSP (`run_mode=5`), use `robstride_send_pos_vel_pp` / `robstride_send_pos_vel_csp`. See [RobStride Parameter Tables](/reference/robstride-parameter-tables#pp--csp-position-modes).
...
| HighTorque | Vendor-native command path (adapter exposed) | Scaling handled automatically under the unified API |
