...
MotorBridge v0.4.1 uses one unified RobStride runtime parameter table from protocol section 4.

</mintly-toc>

MotorBridge keeps the upper-level RobStride API shape shared across RS models. The active runtime parameter table is the protocol section 4 "read/write single parameter" list in the `0x7005..0x702E` range. Older/manual `0x2000` and `0x3000` function-code fields are not split into per-model high-level tables in v0.4.1.
...
- Parameters that must survive power-cycle require type 22 save after writing. In v0.4.1, `robstride-write-param --store 1` performs write, optional verify, then save.
