...
MotorBridge v0.5.0 uses one unified RobStride runtime parameter table from protocol section 4.
</mintly-toc>
...
For RobStride, MotorBridge also writes `zero_sta=1` so the startup
...
RobStride-specific `pos-vel-pp` and `pos-vel-csp` control paths follow the
...
```python
# PP writes vel_max(0x7024) + acc_set(0x7025) + loc_ref(0x7016), run_mode=1.
motor.robstride_send_pos_vel_pp(pos=1.0, vel_max=2.0, acc_set=10.0)
# CSP writes limit_spd(0x7017) + loc_ref(0x7016), run_mode=5.
motor.robstride_send_pos_vel_csp(pos=1.0, vlim=2.0)
```

<Note>
PP and CSP both write the position target to `loc_ref` (`0x7016`), but their
velocity/acceleration registers differ — see
[API: Motor](/api/motor#robstride_send_pos_vel_pppos-vel_max-acc_set-robstride).
