...
- `motor_abi_capabilities_json()` returns a stable JSON capability document for
  supported transports, vendors, lifecycle operations, control modes, and
...
- `motor_abi_version()` — returns the loaded ABI version string.
- `motor_abi_capabilities_json()` — returns a JSON string describing
  supported transports, vendors, modes, and ops.

These mirror the Python helpers `motorbridge.abi_version()` /
...
- `motor_handle_robstride_send_pos_vel_pp` (PP sequence)
- `motor_handle_robstride_send_pos_vel_csp` (CSP sequence)
...
1. Transport constructor:
   ...
   - `motor_controller_new_dm_device(dm_device_type, dm_channel)` (Damiao-only, DM_Device SDK: `USB2CANFD` / `USB2CANFD_DUAL` / `LINKX4C`)
