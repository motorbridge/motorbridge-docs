...
<Note>Source: `bindings/cpp/README.md`</Note>
...
> Chinese version: [README.zh-CN.md](/zh/source/cpp/overview)
...
int main() {
  motorbridge::Controller ctrl("can0");
  auto motor = ctrl.add_robstride_motor(127, 0xFD, "rs-06"); // replace with the RS model you use for limits/logging
...
RobStride supports `rs-00` through `rs-06`. The C++ wrapper keeps one `add_robstride_motor(...)` shape, but parameter read/write is backed by the common section 4 runtime table.
