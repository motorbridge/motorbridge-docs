...
<Note>Source: `bindings/python/README.md`</Note>
...
> Chinese version: [README.zh-CN.md](/zh/source/python/overview)

## README Navigation (What Each One Is For)

If this is your first time in this folder, read in this order:

1. This file: [README.md](/source/python/overview)
   Purpose: Python binding overview (install, API scope, common commands).
2. [examples/README.md](/source/python/examples/overview) (English) / [examples/READMEzh_cn.md](/zh/source/python/examples/overview) (Chinese)
   Purpose: practical demo index and run instructions (from simplest to advanced).
2.5. [`../motorbridge-docs`](https://github.com/tianrking/motorbridge/tree/main/../motorbridge-docs)
   Purpose: canonical Mintlify documentation site (tutorial + API style docs).
3. [get_started/README.md](/source/python/get-started/overview) / [get_started/README.zh-CN.md](/zh/source/python/get-started/overview)
   Purpose: pip-first onboarding path (install -> scan -> run).
4. [DAMIAO_PYTHON_REFERENCE.zh-CN.md](/zh/source/python/damiao-python-reference)
   Purpose: Damiao Python interface reference (parameter lookup style).
5. [DAMIAO_binding.md](/source/python/damiao-binding)
   Purpose: Damiao binding implementation notes (design/internal behavior).
6. [README.zh-CN.md](/zh/source/python/overview)
...
- Version checks are available through `motorbridge.__version__`, `motorbridge.get_version()`, and `motorbridge-cli -v`.
...
- `0.4.1` adds ABI capability/version discovery and keeps Python/C++ binding
  parity tracked in `bindings/api_surface.json`.
...
- RobStride supports `rs-00` through `rs-06`; pass the real model to `add_robstride_motor(...)` because parameter read/write uses the common section 4 runtime parameter table.
...
with Controller("can0") as ctrl:
    motor = ctrl.add_robstride_motor(127, 0xFD, "rs-06")  # replace with the RS model you use for limits/logging
...
motorbridge-cli robstride-read-param \
  --channel can0 --model rs-06 --motor-id 127 --param-id 0x7019 --type f32
...
See [examples/README.md](/source/python/examples/overview) (English) or [examples/READMEzh_cn.md](/zh/source/python/examples/overview) (Chinese).
...
- Full Damiao tuning reference stays in:
  - [DAMIAO_API.md](/source/python/damiao-api)
  - [DAMIAO_API.zh-CN.md](/zh/source/python/damiao-api)
