...
[![Platforms](https://img.shields.io/badge/Platforms-Linux%20%7C%20Windows%20%7C%20macOS-6f42c1.svg)](/zh/source/repository/overview#发布与安装总览完整矩阵)
...
> English version: [README.md](/source/repository/overview)

## 配套仓库

- `motorbridge-studio`：https://github.com/motorbridge/motorbridge-studio
  基于 `ws_gateway` 的独立 Web 控制台。当前发布：`v1.1.3`（2026-07-23）。由 `tools/factory_calib_ui_ws` 拆分而来。

<Note>
`motorbridge-studio` 独立于本文档站演进。其变更日志、能力与发布说明请直接查看该仓库。
</Note>
...
- RobStride:
  ...
  - 型号建议按真实电机指定：`--model` 用于限值/日志；参数读写使用 RobStride 第 4 章通用运行参数表
...
- RobStride RS00-RS06 保持同一套统一控制命令形态。运行参数读写使用第 4 章通用表（`0x7005..0x702E`）；`--model` 仍作为真实型号提示用于限值/日志。
...
- [`motor_core`](https://github.com/motorbridge/motorbridge/tree/main/motor_core): 与厂商无关的控制器、路由、CAN 总线层（Linux SocketCAN / Windows 实验性 PCAN）
- [`motor_vendors/damiao`](https://github.com/motorbridge/motorbridge/tree/main/motor_vendors/damiao): Damiao 协议 / 型号 / 寄存器
- [`motor_vendors/hexfellow`](https://github.com/motorbridge/motorbridge/tree/main/motor_vendors/hexfellow): Hexfellow CANopen-over-CAN-FD 实现
- [`motor_vendors/hightorque`](/source/vendors/hightorque/overview): HighTorque 原生 ht_can 协议实现
- [`motor_vendors/robstride`](https://github.com/motorbridge/motorbridge/tree/main/motor_vendors/robstride): RobStride 扩展 CAN 协议 / 型号 / 参数
- [`motor_vendors/myactuator`](https://github.com/motorbridge/motorbridge/tree/main/motor_vendors/myactuator): MyActuator CAN 协议实现
- [`motor_cli`](/source/rust-cli/overview): 统一 Rust CLI
  - 全参数英文文档: [`motor_cli/README.md`](/source/rust-cli/overview)
  - 全参数中文文档: [`motor_cli/README.zh-CN.md`](/zh/source/rust-cli/overview)
  - Damiao 指令/寄存器文档: [`motor_cli/DAMIAO_API.md`](/source/rust-cli/damiao-api), [`motor_cli/DAMIAO_API.zh-CN.md`](/zh/source/rust-cli/damiao-api)
  - RobStride 指令/参数文档: [`motor_cli/ROBSTRIDE_API.md`](/source/rust-cli/robstride-api), [`motor_cli/ROBSTRIDE_API.zh-CN.md`](/zh/source/rust-cli/robstride-api)
  - MyActuator 指令/模式文档: [`motor_cli/MYACTUATOR_API.md`](/source/rust-cli/myactuator-api), [`motor_cli/MYACTUATOR_API.zh-CN.md`](/zh/source/rust-cli/myactuator-api)
- [`motor_abi`](https://github.com/motorbridge/motorbridge/tree/main/motor_abi): 稳定 C ABI
...
RobStride 使用时请把 `--model rs-06` 替换为真实型号（`rs-00` 到 `rs-06`），用于限值/日志。`read-param` / `write-param` 使用第 4 章通用运行参数表。
...
- [`docs/zh/operation_manual.md`](/zh/source/project/operation-manual)
