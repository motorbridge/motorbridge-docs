---
title: "Python Binding 总览"
---

<Note>Source: `bindings/python/README.zh-CN.md`</Note>
...
1. 本文档 [README.zh-CN.md](/zh/source/python/overview)
   作用：Python binding 总览（安装、API 范围、常用命令）。
2. [examples/READMEzh_cn.md](/zh/source/python/examples/overview)（中文） / [examples/README.md](/source/python/examples/overview)（英文）
...
3. [get_started/README.zh-CN.md](/zh/source/python/get-started/overview) / [get_started/README.md](/source/python/get-started/overview)
...
6. [README.md](/source/python/overview)
   作用：英文版总览（给英文协作成员）。
...
- 当前目标包版本：`0.4.1`。
- 版本可通过 `motorbridge.__version__`、`motorbridge.get_version()` 和 `motorbridge-cli -v` 获取。
- ABI 元数据 helper：
  - `motorbridge.abi_version()` 返回当前加载的 ABI 库版本。
  - `motorbridge.abi_capabilities()` 返回当前加载 ABI 的能力 JSON，并转换为 Python `dict`。
- `0.4.1` 新增 ABI 版本/能力发现，并通过 `bindings/api_surface.json` 跟踪 Python/C++ binding parity。
...
- RobStride 支持 `rs-00` 到 `rs-06`；调用 `add_robstride_motor(...)` 时请传真实型号，因为参数读写使用第 4 章通用运行参数表。
...
with Controller("can0") as ctrl:
    motor = ctrl.add_robstride_motor(127, 0xFD, "rs-06")  # 请替换为你的 id / 用于限值/日志的 RS 型号
    ctrl.enable_all()
    motor.ensure_mode(Mode.MIT, 1000)
...
Python CLI 和 SDK 调用都请使用真实 RobStride 型号（`rs-00` 到 `rs-06`）。系列内控制方法一致；v0.4.1 参数读写继续使用第 4 章通用运行表。
...
详细见 [examples/READMEzh_cn.md](/zh/source/python/examples/overview)（中文）或 [examples/README.md](/source/python/examples/overview)（英文）。
