...
- `motor_abi_capabilities_json()` 返回稳定 JSON 能力文档，描述已支持的 transport、vendor、生命周期操作、控制模式和厂商扩展能力。
...
ABI 暴露元数据入口，便于工具、网关与各语言绑定发现当前加载的 ABI 版本与所支持的
传输/厂商/能力：

- `motor_abi_version()` — 返回当前加载的 ABI 版本字符串。
- `motor_abi_capabilities_json()` — 返回描述所支持传输、厂商、模式与操作的 JSON 字符串。

它们与 Python helper `motorbridge.abi_version()` / `motorbridge.abi_capabilities()` 以及
C++ helper `motorbridge::abi_version()` / `motorbridge::abi_capabilities_json()` 镜像对应。
权威的绑定/API 接口面清单为 `bindings/api_surface.json`，用于保持 ABI、Python、C++ 与文档对齐。
...
- `motor_handle_robstride_send_pos_vel_pp`（PP 序列）
- `motor_handle_robstride_send_pos_vel_csp`（CSP 序列）
...
1. 选择传输层构造器：
   ...
   - `motor_controller_new_dm_device(dm_device_type, dm_channel)`（仅 Damiao，DM_Device SDK：`USB2CANFD` / `USB2CANFD_DUAL` / `LINKX4C`）
