...
| RobStride | 必须传真实型号：`rs-00` 到 `rs-06` | `motor-id` 必传；`feedback-id` 常用 `0xFD` | `ping`、`scan`、`mit`、`vel`、`read-param`、`write-param`、`save` |
...
| `--version`, `-v` | flag | 关闭 | 输出 Rust CLI 包版本并退出 |
...
| `--model` | string | 按 vendor 决定 | Damiao 默认 `4340`；RobStride 必须使用真实 `rs-00` 到 `rs-06` 型号；HighTorque 默认 `hightorque`；MyActuator 默认 `X8` |
...
| `--store` | `0/1` | 改 ID 默认 `1`，写参默认 `0` | 改 ID / write-param | 改 ID 后保存，或参数写入成功后保存 |
...
- RobStride 必须按真实电机传 `--model`。基础控制命令的上层形态一致，但原生功能码/参数表使用第 4 章通用运行参数；同一个功能码在不同型号上可能对应不同名称或类型。
...
motor_cli \
  --vendor robstride --channel can0 --model rs-06 --motor-id 20 --feedback-id 0xFD \
  --mode write-param --param-id 0x7005 --param-value 2 --store 1

# 改 ID（旧 1 -> 新 11）并存参
motor_cli \
  --vendor robstride --channel can0 --model rs-06 --motor-id 1 --feedback-id 0xFD \
...
# 设零（实验时序）
motor_cli \
  --vendor robstride --channel can0 --model rs-06 --motor-id 11 --feedback-id 0xFD \
