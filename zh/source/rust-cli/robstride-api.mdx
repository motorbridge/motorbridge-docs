...
> English version: [ROBSTRIDE_API.md](/source/rust-cli/robstride-api)
...
这些型号的高层控制调用形态保持一致。运行参数 `read-param` / `write-param` 使用协议第 4 章通用表（`0x7005..0x702E`）；`--model` 仍请填写真实 RS 型号，用于限值和日志。

运行参数表对齐 RobStride 手册第 4 章“可读写单个参数列表”。
...
参数访问是使用第 4 章通用运行参数的。`0x7000` 段的常用运行控制参数由统一控制路径共用；RobStride 手册里的功能码参数，尤其是 `0x2000` 和 `0x3000` 段，同一个 ID 在不同 RS 型号上可能对应不同名称、数据类型或范围。
...
motor_cli \
  --vendor robstride --channel can0 --model rs-06 --motor-id 127 --feedback-id 0xFD \
  --mode write-param --param-id 0x700A --param-value 0.3 --store 1
```
...
1. `pos-vel` 参数生效性在部分固件上不稳定：
   ...
   - 当前 `MIT` 路径相对更稳定。
2. RobStride 置零已保持统一上层命令，并在内部写 `zero_sta(0x7029)=1`：
   - 正常使用 `zero` / `set_zero_position()`；
   - 如果需要掉电后仍保持 `-pi..pi` 上电区间，请配合 `store_parameters` / `--store 1`。
