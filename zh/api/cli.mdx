...
| `--dm-device-type` | — | DM_Device 适配器类型（用于 dm-device 传输）：usb2canfd, usb2canfd-dual, linkx4c |
| `--dm-channel` | — | DM_Device 通道。usb2canfd: 0；usb2canfd-dual: 0/1；linkx4c: 0..3。省略则扫描所有通道。 |
| `--model` | `4340` | 电机型号字符串 |
