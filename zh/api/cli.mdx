...
| `--dm-device-type` | `usb2canfd-dual` | DM_Device 适配器类型（用于 dm-device 传输）：usb2canfd, usb2canfd-dual, linkx4c |
| `--dm-channel` | `0` | DM_Device 通道。usb2canfd: 0；usb2canfd-dual: 0/1；linkx4c: 0/1/2/3。`scan` 时省略则扫描所有通道。 |
| `--model` | `4340` | 电机型号字符串 |
| `--motor-id` | `0x01` | 电机 CAN ID（支持十六进制） |
| `--feedback-id` | `0x11` | 反馈 CAN ID（支持十六进制） |
| `--mode` | `mit` | 控制模式：enable, disable, mit, pos-vel, pos-vel-pp, pos-vel-csp, vel, force-pos, ping, zero, set-zero, firmware-version, save, zero-by-offset, read-param, write-param |
| `--loop` | `100` | 控制迭代次数 |
| `--dt-ms` | `20` | 控制周期（毫秒） |
