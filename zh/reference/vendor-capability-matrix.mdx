...
| 厂商 | 添加方法 | 典型传输 | 协议族 |
|------|----------|----------|--------|
| Damiao | `add_damiao_motor()` | SocketCAN, DM 串口, DM-Device | DM-J4310/4340 |
| RobStride | `add_robstride_motor()` | SocketCAN | RobStride 扩展 CAN |
| RobStride (CiA402, 实验) | `add_robstride_motor(..., vendor="robstride_cia402")` | SocketCAN | CANopen/CiA402（`F_CMD=1`） |
| RobStride (MIT, 实验) | `add_robstride_motor(..., vendor="robstride_mit")` | SocketCAN | RobStride MIT（`F_CMD=2`） |
...
| RobStride | rs-00, rs-01, rs-06 | ✅ | ✅ (= PP；另有 CSP) | ✅ | ❌ |
| MyActuator | X8 | ❌ | ✅ | ✅ | ❌ |
...
- `POS_VEL` 映射到原生位置流程，即 PP（`run_mode=1` + `loc_ref` + `limit_spd` + 可选 `loc_kp`）。统一 `pos-vel` 走的是这条 PP 路径（经 `limit_spd`）。
- 统一 `pos-vel` 下，CLI 的 `--vel/--kd/--tau` 为无效参数：只给 warning，不报错。
- 扭矩/电流目前仍是参数级能力（`write-param`），尚未独立成统一模式（因此 `FORCE_POS` 为 ❌）。
- **PP / CSP：** 统一 `pos-vel` 已是 PP（经 `limit_spd`）。需完整 PP 参数（`vel_max` + `acc_set`）或 CSP（`run_mode=5`）时，改用 `robstride_send_pos_vel_pp` / `robstride_send_pos_vel_csp`。详见 [RobStride 参数表](/zh/reference/robstride-parameter-tables#pp--csp-位置模式)。
- **固件版本：** 支持 RobStride 固件版本请求与应答解码（`--mode firmware-version`）。
- **mode-aware 受控停止：** `handle_stop` 已加固，执行按模式区分的受控停止。
- **实验性厂商路径：** `robstride_cia402`（CANopen/CiA402，`F_CMD=1`）与 `robstride_mit`（MIT，`F_CMD=2`）暴露核心 CLI 入口与文档，但**尚未达到量产可用**——真机验证矩阵、高速/控制人机工程、EDS/PDO/SYNC 覆盖、`dm-device` 传输接线仍待补。
...
> 这些厂商原生命令在统一四模式之外提供，主要用于现场调试与协议联调。
...
| HighTorque | 厂商原生命令路径（由适配层暴露） | 统一接口下自动完成缩放映射 |
