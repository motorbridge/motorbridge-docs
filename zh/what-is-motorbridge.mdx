...
<Badge variant="primary">v0.4.1</Badge> <Badge variant="secondary">Rust 核心</Badge> <Badge variant="outline">Python 绑定</Badge>
...
```mermaid
flowchart TB
  App["用户应用"] --> Bindings["语言绑定"]
  Bindings --> Python["Python 包"]
  Bindings --> CPP["C++ 封装"]
  Bindings --> Gateway["WS / ROS2 集成"]
  Python --> ABI["C ABI: libmotor_abi"]
  CPP --> ABI
  Gateway --> ABI
  ABI --> Core["Rust 核心: controller, bus, models"]
  Core --> Vendors["厂商驱动"]
  Vendors --> Damiao["达妙"]
...
<Note>
上方是适合文档阅读的系统图。下方保留终端友好的文本图，方便复制到纯文本环境。
</Note>
