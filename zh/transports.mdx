...
| `Controller.from_dm_device(dm_device_type, dm_channel)` | DM_Device SDK | 达妙 USB2CANFD / USB2CANFD_DUAL / LINKX4C |
...
基于达妙 DM_Device SDK 的原生传输，用于 `USB2CANFD`、`USB2CANFD_DUAL` 与 `LINKX4C` 适配器。适配器须为 USB 模式；该传输仅用于达妙电机。
...
| `dm_channel` | int 或 None | None | 通道号。`usb2canfd` 用 `0`；`usb2canfd-dual` 用 `0`/`1`；`linkx4c` 用 `0..3`。省略则扫描所选适配器的所有通道。 |
...
Python wheel 不内嵌厂商运行时。若 DM_Device 运行时缺失，MotorBridge 会打印所需运行时文件、其 GitHub 下载 URL、放置位置，以及平台 ABI 要求。
...
<Note>
Windows DM_Device 运行时仅限 x86_64。长时间运行的进程会复用已打开的适配器，避免重复打开。
</Note>
...
│
├── 带 USB2CANFD / USB2CANFD_DUAL / LINKX4C 的达妙 ──→ from_dm_device
│
