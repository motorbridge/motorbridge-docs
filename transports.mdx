...
| `dm_channel` | int or None | None | Channel index. `usb2canfd` uses `0`; `usb2canfd-dual` uses `0`/`1`; `linkx4c` uses `0..3`. Omit to scan every channel for the selected adapter. |
