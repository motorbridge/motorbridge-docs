...
| `--transport` | `auto` | Transport: auto, socketcan, socketcanfd, dm-serial, dm-device |
| `--serial-port` | `/dev/ttyACM0` | Serial port (for dm-serial transport) |
| `--serial-baud` | `921600` | Serial baud rate |
| `--dm-device-type` | `usb2canfd-dual` | DM_Device adapter type (for dm-device transport): usb2canfd, usb2canfd-dual, linkx4c |
| `--dm-channel` | `0` | DM_Device channel. usb2canfd: 0; usb2canfd-dual: 0/1; linkx4c: 0/1/2/3. Omit on `scan` to scan all channels. |
| `--model` | `4340` | Motor model string |
| `--motor-id` | `0x01` | Motor CAN ID (hex supported) |
| `--feedback-id` | `0x11` | Feedback CAN ID (hex supported) |
| `--mode` | `mit` | Control mode: enable, disable, mit, pos-vel, pos-vel-pp, pos-vel-csp, vel, force-pos, ping, zero, set-zero, firmware-version, save, zero-by-offset, read-param, write-param |
...
store_parameters sent (via new id)
