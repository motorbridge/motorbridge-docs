...
<Note>
**RobStride PP / CSP:** RobStride also exposes `robstride_send_pos_vel_pp`
...
replies do not synthesize a `MotorState`, so `request_feedback() ->
get_state()` will not appear to refresh state when it cannot. Use the correct
query for your goal:
...
**Mode-aware controlled stop:** `handle_stop` is hardened and
performs a mode-aware controlled stop instead of a bare disable.
</Note>
