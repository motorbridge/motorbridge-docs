...
| Name | Type | Default | Description |
|------|------|---------|-------------|
| `pos` | `float` | required | Target position (radians) → writes `loc_ref` (`0x7016`) |
| `vel_max` | `float` | required | Max velocity (rad/s) → writes `vel_max` (`0x7024`) |
| `acc_set` | `float` | required | Acceleration (rad/s²) → writes `acc_set` (`0x7025`) |
...
|------|------|---------|-------------|
| `pos` | `float` | required | Target position (radians) → writes `loc_ref` (`0x7016`) |
| `vlim` | `float` | required | Velocity limit (rad/s) → writes `limit_spd` (`0x7017`) |
